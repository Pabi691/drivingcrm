...
export default function EnquiriesPage({ user, onLogout }: Props) {
  ...
  const filtered = enquiries.filter((item) => item.name.toLowerCase().includes(search.toLowerCase()));

  const handleDelete = (id: number) => {
    setEnquiries(enquiries.filter((i) => i.id !== id));
  };

  const handleSave = () => {
    if (!editItem) return;
    if (editItem.id === 0) {
      setEnquiries([...enquiries, { ...editItem, id: Date.now() }]);
    } else {
      setEnquiries(enquiries.map((i) => (i.id === editItem.id ? editItem : i)));
    }
    setEditItem(null);
    setShowModal(false);
  };
  ...
  return (
    <DashboardLayout user={user} onLogout={onLogout}>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={openAdd} className="px-4 py-2 bg-blue-600 text-white rounded-lg">+ Add Enquiry</button>
      </div>

      <input placeholder="Search enquiry..." className="w-full border rounded-lg px-4 py-2 mb-4" value={search} onChange={(e) => setSearch(e.target.value)} />
      ...
      {showModal && editItem && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center">
          <div className="bg-white p-6 w-[400px] rounded-xl shadow-xl">
            <h3 className="text-xl font-semibold mb-4 text-gray-800 border-gray-300">{editItem.id === 0 ? "Add Enquiry" : "Edit Enquiry"}</h3>
